...
export default function runResourcesPromptsTests() {
  ...
  class MockMcpServer {
    ...
    resource(uri, mimeType, metadata, handler) {
  ...
  test('Should register API documentation resource', () => {
    ...
    const resource = server.resources.get('absmartly://docs/api');
...
${customFields.map(f => `- ${f.name} (${f.type})`).join('\n')}`
    ...
    const result = await server.getResource('absmartly://docs/api');
  ...
  test('Should register experiment template resource when custom fields exist', () => {
    ...
    const resource = server.resources.get('absmartly://templates/experiment');
  ...
  test('Should return correct prompt content', async () => {
    ...
    const result = await server.getPrompt('experiment-status');
...
Let's start with the experiment basics.`
    ...
    const result = await server.getPrompt('create-experiment');
  ...
  test('Should throw error for non-existent resource', async () => {
    ...
    try {
      ...
      assertContains(error.message, 'Resource not found', 'Should have correct error message');
    }
    return true;
  });
  ...
  test('Should throw error for non-existent prompt', async () => {
    ...
    try {
      ...
      assertContains(error.message, 'Prompt not found', 'Should have correct error message');
    }
    return true;
  });
